-- ----------------------------
-- RuoYi 4.8.3 PostgreSQL 版建表脚本
-- 由 sql/ry_20260319.sql（MySQL）转换而来，MySQL 原脚本保留不动
-- 转换要点：
--   bigint(20)/int(n) -> int8/int4，datetime -> timestamp，blob -> bytea，longblob -> text
--   auto_increment    -> serial/bigserial + setval 对齐初始自增值
--   内联 comment      -> comment on table / comment on column（代码生成器依赖列注释）
--   inline key        -> 独立 create index
--   sysdate()         -> now()，MySQL 的 \' 转义 -> PG 的 ''
-- ----------------------------
...
select setval('sys_post_post_id_seq', 5, false);
...
select setval('sys_oper_log_oper_id_seq', 100, false);
...
select setval('sys_dict_type_dict_id_seq', 100, false);
...
select setval('sys_dict_data_dict_code_seq', 100, false);
...
select setval('sys_logininfor_info_id_seq', 100, false);
...
select setval('sys_job_job_id_seq', 100, false);
...
select setval('sys_notice_notice_id_seq', 10, false);
